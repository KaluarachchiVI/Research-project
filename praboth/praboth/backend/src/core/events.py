...
class PermissionGuard:
    ...
    def allow(self, event: Event) -> bool:
        ...
        if self.privacy_pause:
            self.logger.debug("Event from %s blocked by permissions: privacy_pause", event.source)
            return False
        if not self.consent_granted:
            self.logger.debug("Event from %s blocked by permissions: consent not granted", event.source)
            return False

        if event.source not in self.allowed_sources:
            self.logger.debug(
                "Event from %s blocked by permissions: source not in allowed_sources=%s",
                event.source,
        ...
        if self.idle_block_seconds and self._last_idle_seconds >= self.idle_block_seconds:
            self.logger.debug(
                "Event from %s blocked by permissions: idle %.0fs >= %ss",
                event.source,
                self._last_idle_seconds,
        ...
        if blocked:
            self.logger.debug(
                "Event from %s blocked by permissions: context_blocked=%s",
                event.source,
                blocked,
            )
            return False
