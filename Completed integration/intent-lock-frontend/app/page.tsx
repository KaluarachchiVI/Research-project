...
export default function Home() {
  ...
  const resetSession = () => {
    void getDesktopBridge()?.lockdown.stop({ reason: "reset" });
    ...
    if (SCHEDULER_ENABLED) {
      ...
      startTimeBlockSession({
        ...
        .then((res) => {
          ...
          if (firstInterval) {
            const it = firstInterval.interval_type ?? "work";
            setCurrentIntervalType(it);
            setWorkDuration(firstInterval.work_interval ?? 30);
            setBreakDuration(firstInterval.break_duration ?? 5);
            void getDesktopBridge()?.lockdown.setPhase(it, {
              workMinutes: firstInterval.work_interval ?? 30,
            });
          } else {
            setCurrentIntervalType("work");
            setWorkDuration(30);
  ...
  const requestLandingSessionStart = () => {
    if (isDesktopApp()) {
      router.push(`/focus-setup?block=${String(configBlockMinutes)}`);
      return;
    }
    startSession();
  };
    ...
    try {
      ...
      if (next) {
        ...
        if (currentIntervalType === "work") {
          ...
          void getDesktopBridge()?.lockdown.setPhase("break");
        } else {
          const wm =
            typeof next.work_interval === "number" ? next.work_interval : workDuration;
          if (typeof next.work_interval === "number") {
            setWorkDuration(next.work_interval);
          }
          setCurrentIntervalType("work");
          void getDesktopBridge()?.lockdown.setPhase("work", { workMinutes: wm });
        }
      ...
      {!showDashboard && (
        <div className="mx-auto max-w-4xl">
          ...
          <SessionConfig
            ...
            onStartSession={requestLandingSessionStart}
                ...
                <TimerCard
                  ...
                  onToggleWorkMode={() =>
                    setCurrentIntervalType((t) => {
                      const next = t === "work" ? "break" : "work";
                      setTimeout(() => {
                        if (next === "work") {
                          void getDesktopBridge()?.lockdown.setPhase("work", {
                            workMinutes: workDuration,
                          });
                        } else {
                          void getDesktopBridge()?.lockdown.setPhase("break");
