...
export async function fetchPendingPrompt(): Promise<{
  prompt: { prompt_id: number; reason: string; issued_at: string } | null;
}> {
  return getJSON<{
    prompt: { prompt_id: number; reason: string; issued_at: string } | null;
  }>("/ema/pending");
...
export async function submitEmaResponse(
  ...
  return postEmaResponse(promptId, rating, disposition, note);
}

export async function fetchPolicyEvents(limit = 100): Promise<PolicyEvent[]> {
  const payload = await getJSON<{ events: PolicyEvent[] }>(
    `/policy/events?limit=${limit}`
  );
...
export async function updateContextBlocklist(
  entries: string[]
...
export async function fetchDistractionHistory(
  limit = 50
): Promise<DistractionPeriod[]> {
  const payload = await getJSON<{ periods: DistractionPeriod[] }>(
    `/distractions?limit=${limit}`
  );
