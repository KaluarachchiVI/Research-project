...
class EstimatorService:
    ...
    def latest_payload(self) -> Optional[Dict[str, Any]]:
        ...
        if not state or not state.estimate:
            return None
        est = state.estimate
        # The Kalman latent state is not guaranteed to stay in [0, 1]. Map it into [0, 1]
        # so clients see continuous changes rather than saturating at 0/1.
        try:
        ...
        if load_raw != load_raw:  # NaN
            ...
            # Prefer mapping relative to recent history (running mean/std of raw load),
            # which avoids "stuck at ~1.0" when load_raw is consistently high.
            ...
            if len(mean_list) == 1 and len(var_list) == 1:
                ...
                # Fallback: if raw already in [0,1], pass it through; otherwise use a scaled sigmoid.
                if 0.0 <= load_raw <= 1.0:
                    load = load_raw
                else:
