...
def create_app(config_path: Optional[Path] = None) -> FastAPI:
    ...
    # Allow browser clients (Intent-Lock and any local dev). Use * so 127.0.0.1 and localhost both work.
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def root() -> Dict[str, str]:
        return {"service": "Cognitive Load Estimator", "docs": "/docs", "estimate": "/estimate"}
    ...
    async def latest_estimate(svc: EstimatorService = Depends(get_service)) -> Dict[str, Any]:
        ...
        if not payload:
            # Cold start: no estimate yet. Return 200 with default load so clients get a valid response.
            return {
                "load": 0.5,
                "warming": True,
                "hop_index": 0,
