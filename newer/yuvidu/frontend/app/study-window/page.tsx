...
interface StudyWindowPrediction {
  ...
  confidence: number;
  score: number;
  alternatives: Array<{ time_range: string; score: number }>;
  current_context: {
    current_time: string;
    current_day: string;
    data_points: number;
...
export default function StudyWindowPage() {
  ...
  const fetchStudyWindow = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`${YUVIDU_API_BASE}/next-best-study-window`);
      if (!response.ok) throw new Error(`Server error: ${response.status}`);
      const data = await response.json();
      setStudyWindow(data.prediction ?? null);
      setError(null);
    } catch (err) {
          ...
          <div className="study-window-card">
            <div className="study-window-main">
              <div className="study-window-time">
                ...
                <p className="study-window-duration">
                  Duration: {studyWindow.best_window.duration_hours} hours
                </p>
              </div>
