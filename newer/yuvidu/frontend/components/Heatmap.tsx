...
export default function Heatmap({ percentages, hourlyData }: HeatmapProps) {
  const hours = [
  ...
  const hourlyDataForDisplay = hourlyData && hourlyData.length > 0
    ...
    : hours.map((hour, index) => {
        let value: number;
        if (index < 6) value = percentages?.morning ?? 0;
        else if (index < 12) value = percentages?.afternoon ?? 0;
        else if (index < 18) value = percentages?.evening ?? 0;
        else value = percentages?.night ?? 0;
        return { hour, value };
      });
  ...
  const colorScale = scaleLinear<string>()
    ...
    .range([
      "#dc2626", "#f87171", "#fbbf24", "#facc15", "#84cc16", "#16a34a"
    ]);

  return (
    <div className="heatmap-container">
      <h3 className="heatmap-title">🌡️ Prediction Intensity Heatmap</h3>
      <div className="heatmap-grid-section">
        ...
        <div className="heatmap-cells">
          ...
          {hourlyDataForDisplay.map((data, index) => {
            const intensity = "intensity" in data ? data.intensity : (data as { value?: number }).value ?? 0;
            ...
            return (
              ...
              >
                <div className="heatmap-cell-content">
                  <div className="heatmap-cell-hour">{data.hour}</div>
                  <div className={`heatmap-cell-percentage ${textColor}`}>{intensity.toFixed(0)}%</div>
                </div>
              </div>
            );
          })}
      ...
      <div className="heatmap-summary">
        <div className="heatmap-summary-card heatmap-summary-morning">
          ...
          <div className="heatmap-summary-value">{(percentages?.morning ?? 0).toFixed(1)}%</div>
        </div>
        <div className="heatmap-summary-card heatmap-summary-afternoon">
          <div className="heatmap-summary-label">Afternoon</div>
          <div className="heatmap-summary-value">{(percentages?.afternoon ?? 0).toFixed(1)}%</div>
        </div>
        <div className="heatmap-summary-card heatmap-summary-evening">
          <div className="heatmap-summary-label">Evening</div>
          <div className="heatmap-summary-value">{(percentages?.evening ?? 0).toFixed(1)}%</div>
        </div>
        <div className="heatmap-summary-card heatmap-summary-evening">
          <div className="heatmap-summary-label">Night</div>
          <div className="heatmap-summary-value">{(percentages?.night ?? 0).toFixed(1)}%</div>
        </div>
      </div>
      <div className="heatmap-legend">
        <div className="heatmap-legend-title">Intensity Scale (Low → High)</div>
        <div className="heatmap-legend-container">
          ...
          <div className="heatmap-legend-bar">
            {[0, 20, 40, 60, 80, 100].map((val, idx) => {
              ...
              return (
                <div key={idx} className={`heatmap-legend-segment ${textColor}`} style={{ background: colorScale(val) }}>
                  {val}%
                </div>
              );
            })}
